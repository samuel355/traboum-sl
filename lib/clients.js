...
export async function fetchPendingPlotAssignments(db) {
  const assignments = [];
  const pageSize = 1000;
  ...
  for (let from = 0; ; from += pageSize) {
    ...
    (data ?? []).forEach((plot) => {
      const clientId = plot.properties?.assignedClientId;
      if (clientId) {
        assignments.push({
          clientId,
          plotId: String(plot.id),
          plotNumber: plot.properties?.plotNumber ?? plot.properties?.Plot_No ?? "",
          streetName: plot.properties?.streetName ?? plot.properties?.Street_Nam ?? "",
        });
      }
    });
...
export function summarizeClientRecords({ allocations = [], reservations = [], transfers = [], pendingPlots = [] }) {
  ...
  pendingPlots.forEach((plot) => {
    if (plot.plotId) plotIds.add(plot.plotId);
  });

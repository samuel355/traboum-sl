...
export async function fetchAllPlots() {
  ...
  const [{ data: allocations, error: allocationsError }, { data: transfers, error: transfersError }] = await Promise.all([
    supabase
  ...
  const currentClientByPlot = new Map();
  (allocations ?? []).forEach((row) => {
    if (!currentClientByPlot.has(String(row.plot_id))) {
      currentClientByPlot.set(String(row.plot_id), {
  ...
  (transfers ?? []).forEach((row) => {
    ...
    const existing = currentClientByPlot.get(key);
    if (!existing || new Date(row.created_at) > new Date(existing.date)) {
      currentClientByPlot.set(key, { name: row.new_client_name, date: row.created_at });
    }
  });

  return all.map((plot) => ({
    ...plot,
    currentClientName: currentClientByPlot.get(String(plot.id))?.name ?? null,
  }));
}
